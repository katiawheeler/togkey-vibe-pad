...
enum custom_keycodes {
    ...
    TV_RESUME,                    // Key 4: /resume
    TV_CLEAR,                     // Key 5: /clear
    TV_COMPACT,                   // Key 6: /compact
    TV_PARTY,                     // Encoder push: RGB party mode + type "claude"
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [0] = LAYOUT(
        TV_PARTY,                              // Encoder push: RGB party + "claude"
        TV_MACWHISPER, TV_ENTER,   TV_STOP,    // Row 1: MacWhisper, Enter, ESC
        TV_RESUME,     TV_CLEAR,   TV_COMPACT  // Row 2: /resume, /clear, /compact
    )
};
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case TV_RESUME:
            ...
            if (record->event.pressed) {
                #ifdef RGBLIGHT_ENABLE
                rgblight_setrgb(255, 255, 0);  // Yellow feedback
                #endif
                SEND_STRING("/resume");
            } else {
                #ifdef RGBLIGHT_ENABLE
        ...
        case TV_PARTY:
            // Encoder push: Toggle RGB party mode + type "claude"
            if (record->event.pressed) {
...
void check_encoder_push(void) {
    ...
    if (current_state != encoder_push_state) {
        ...
        if (encoder_push_state) {
            // Pressed - toggle RGB party mode + type "claude"
            rgb_party_mode = !rgb_party_mode;
            #ifdef RGBLIGHT_ENABLE
            if (rgb_party_mode) {
                rgblight_enable();
